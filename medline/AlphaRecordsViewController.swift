...
class AlphaRecordsViewController: UITableViewController {
    ...
    // Persistence
    var alphaReadings : [AlphaReading] = []
    ...
    func loadReadings(){
        let poRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "AlphaReading")
        poRequest.sortDescriptors = [NSSortDescriptor(key: "timestamp", ascending: false)]
        
        var alphaReadings : [AlphaReading]!
        
        do{
            alphaReadings = try context.fetch(poRequest) as! [AlphaReading]
        } catch {
            print("fetching failed")
        }
        
        self.alphaReadings  = alphaReadings
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return alphaReadings.count
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "recordCell", for: indexPath)

        let reading = alphaReadings[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        recordDelegate?.didSelectAlphaReading(reading: alphaReadings[indexPath.row])
        tableView.reloadData()
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            context.delete(alphaReadings[indexPath.row])
            (UIApplication.shared.delegate as! AppDelegate).saveContext()
            alphaReadings.remove(at: indexPath.row)
